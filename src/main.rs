...
fn main() {
    ...
    fn inspect(arg:&String) {
        let string = String::from(arg);
        let ends_with: bool = string.ends_with("s");
        println!("The string ends with an s? {}", ends_with);
        println!("{}", if ends_with { "Is a plural"} else{ "is a singular" })
    }
    ...
    fn bedazzle(str: &mut String) {
        let sparkly = String::from("sparkly");
       *str = sparkly;
    }
}